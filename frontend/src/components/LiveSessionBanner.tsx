...
export const LiveSessionBanner: React.FC<LiveSessionBannerProps> = ({
  ...
  const primaryMeeting = activeLiveMeeting || nextScheduledMeeting || (batchMeetings.length > 0 ? batchMeetings[0] : undefined);

  // Check if an ACTUAL call or Zoom/Meet link has been configured
  const hasRealMeeting = Boolean(
    primaryMeeting &&
    ((primaryMeeting.meetingLink && primaryMeeting.meetingLink.trim().length > 0) ||
     (primaryMeeting.meetingId && primaryMeeting.meetingId.trim().length > 0) ||
     (primaryMeeting.title && primaryMeeting.status === "live"))
  );

  const hasBatchZoom = Boolean(
    batch &&
    (batch as any).zoomConfig &&
    Boolean((batch as any).zoomConfig.meetingLink && (batch as any).zoomConfig.meetingLink.trim().length > 0)
  );

  const hasSettingsZoom = Boolean(
    (settings as any)?.zoomConfig &&
    Boolean((settings as any).zoomConfig.meetingLink && (settings as any).zoomConfig.meetingLink.trim().length > 0)
  );

  const hasActiveCall = hasRealMeeting || hasBatchZoom || hasSettingsZoom;

  const defaultZoomConfig = {
    topic: primaryMeeting?.title || `${batch?.name || "Cohort"} Sync Session`,
    agenda: "Weekly Mentorship & Project Review",
    instructorName: "Program Mentor",
    meetingId: "",
    meetingLink: "",
    passcode: "",
    scheduledDate: "Today",
    ...
    summary: {
      duration: "1h 00m",
      overview: "Weekly cohort check-in and technical milestones review.",
      keyHighlights: [
        "Sprint milestone progress reviews",
        "Architecture and codebase guidance",
      ],
      actionItems: [
        "Complete technical assignments on schedule",
        "Review module feedback with mentors",
      ],
    },
  };
  ...
  const zoomConfig = primaryMeeting
    ? {
        ...
        scheduledDate: primaryMeeting.scheduledDate || "Today",
        scheduledTime: primaryMeeting.scheduledTime || "10:00 AM - 11:00 AM",
        ...
        summary: primaryMeeting.summary || defaultZoomConfig.summary,
      }
    : (batch && (batch as any).zoomConfig && (batch as any).zoomConfig.meetingLink) 
    ? (batch as any).zoomConfig 
    : ((settings as any)?.zoomConfig && (settings as any).zoomConfig.meetingLink)
    ? (settings as any).zoomConfig
    : defaultZoomConfig;
  const status = zoomConfig?.status || "scheduled";
  ...
  const handleSaveSchedule = (e: React.FormEvent) => {
    ...
    if (!editMeetingLink.trim()) {
      alert("Please provide a valid Zoom or Google Meet URL link.");
      return;
    }

    const meetingToSave: import("../types").ScheduledMeeting = {
      id: primaryMeeting?.id || `meet_${Date.now()}`,
      batchId: batch?.id || "b1",
      title: editTopic.trim() || `${batch?.name || "Cohort"} Sync Session`,
      agenda: editAgenda.trim(),
      instructorName: editInstructor.trim() || "Program Mentor",
      meetingId: editMeetingId.trim(),
      meetingLink: editMeetingLink.trim(),
      passcode: editPasscode.trim(),
      scheduledDate: editScheduledDate.trim() || "Today",
      scheduledTime: editScheduledTime.trim() || "10:00 AM - 11:00 AM",
      status: editStatus,
    ...
    if (onUpdateScheduledMeeting) {
      onUpdateScheduledMeeting(meetingToSave);
    }
    ...
    const updatedSettings: AppSettings = {
      ...
      zoomConfig: {
        ...settings.zoomConfig,
        topic: meetingToSave.title,
        agenda: meetingToSave.agenda,
        instructorName: meetingToSave.instructorName,
        meetingId: meetingToSave.meetingId,
        meetingLink: meetingToSave.meetingLink,
        passcode: meetingToSave.passcode,
        scheduledDate: meetingToSave.scheduledDate,
        scheduledTime: meetingToSave.scheduledTime,
        status: meetingToSave.status as any,
        recordingUrl: meetingToSave.recordingUrl,
      },
    };
    onUpdateSettings(updatedSettings);
  ...
  const handleDeleteCall = () => {
    if (primaryMeeting && onDeleteScheduledMeeting) {
      onDeleteScheduledMeeting(primaryMeeting.id);
    }
    const updatedSettings: AppSettings = {
      ...settings,
      zoomConfig: {
            ...
            <form onSubmit={handleSaveSchedule} className="space-y-3.5">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. Weekly Mentorship & Code Walkthrough"
              ...
              <div>
                <label className="block text-xs font-bold text-slate-700 uppercase mb-1">
                  Meeting URL / Join Link (Zoom / Google Meet) *
                </label>
                <input
                  type="url"
                  required
              ...
              <div className="flex items-center justify-between gap-2 pt-3 border-t border-slate-100">
                {primaryMeeting && onDeleteScheduledMeeting && (
                  <button
                    type="button"
                    onClick={handleDeleteCall}
                    className="px-3.5 py-2 text-xs font-bold text-rose-600 hover:bg-rose-50 rounded-xl transition cursor-pointer flex items-center gap-1.5"
                  >
                ...
                <div className="flex items-center gap-2 ml-auto">
                  ...
                  <button
                    type="submit"
                    className="px-5 py-2.5 bg-gradient-to-r from-teal-500 to-emerald-500 hover:from-teal-600 hover:to-emerald-600 text-white rounded-xl text-xs font-black shadow-md transition cursor-pointer"
                  >
                    Save & Publish Call
                  </button>
                </div>
  ...
  // If NO call or Zoom link was added:
  if (!hasActiveCall) {
    if (userRole === "student") {
      return null;
    }
    ...
    return (
      <div className="bg-white rounded-3xl border border-slate-200/90 p-5 shadow-xs flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3.5">
          <div className="w-11 h-11 rounded-2xl bg-teal-50 text-teal-600 flex items-center justify-center border border-teal-100 flex-shrink-0">
            <Video className="w-5 h-5" />
          </div>
          <div>
            <div className="flex items-center gap-2">
              <h3 className="text-sm font-black text-slate-800">
                No Live Call or Sync Session Scheduled
              </h3>
            ...
            <p className="text-xs text-slate-500 mt-0.5">
              No active Zoom or Google Meet link is configured for {batch?.name || "this cohort"}. Schedule a session to notify interns in real-time.
            </p>
          </div>
        </div>
        ...
        <button
          type="button"
          onClick={() => {
            setEditTopic(`${batch?.name || "Cohort"} Sync & Mentorship`);
            setEditAgenda("Weekly Progress Review & Live Code Walkthrough");
            setEditInstructor("Program Mentor");
            setEditMeetingLink("");
            setEditMeetingId("");
            setEditPasscode("");
            setEditScheduledDate("Today");
            setEditScheduledTime("10:00 AM - 11:00 AM");
            setEditStatus("scheduled");
            setShowScheduleModal(true);
          }}
          className="px-5 py-2.5 bg-gradient-to-r from-teal-500 to-emerald-500 hover:from-teal-600 hover:to-emerald-600 text-white font-black text-xs rounded-xl shadow-md shadow-teal-500/15 transition flex items-center justify-center gap-1.5 flex-shrink-0 cursor-pointer"
        >
              ...
              <div className="grid grid-cols-2 gap-2 w-full">
                ...
                {userRole === "admin" && (
                  <button
                    type="button"
                    onClick={handleDeleteCall}
                    className="flex items-center justify-center gap-1.5 px-3 py-2 rounded-xl bg-rose-500/15 hover:bg-rose-500/25 text-rose-300 border border-rose-500/30 text-xs font-bold transition cursor-pointer"
                    title="Cancel and remove this call"
                  >
                    <Trash2 className="w-3.5 h-3.5 text-rose-400 flex-shrink-0" />
                    <span className="truncate">Cancel Call</span>
                  </button>
                )}
              </div>
            </div>
