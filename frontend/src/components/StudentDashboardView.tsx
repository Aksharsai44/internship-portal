...
export const StudentDashboardView: React.FC<StudentDashboardViewProps> = ({
  ...
  // Filter only approved or scheduled interviews for the current intern
  const internInterviews = interviewRequests.filter(
    (r) =>
      r.internId === currentStudent.id &&
      (r.status === "approved" || r.status === "scheduled")
  );
  const batchAssignments = assignments.filter((a) => a.batchId === selectedBatch.id && !a.isLocked);
  const allBatchQuestions = liveQuestions.filter((q) => q.batchId === selectedBatch.id && !q.isLocked);
  ...
  const handleCastVote = (poll: LiveQuestion, option: string) => {
    const existing = poll.responses?.find((r) => r.studentId === currentStudent.id);
    if (studentVoteMap[poll.id] || existing) return;

    const isCorrect = poll.type === "mcq" || poll.type === "true_false" ? option === poll.correctAnswer : undefined;

    setStudentVoteMap((prev) => ({ ...prev, [poll.id]: option }));

    if (onSubmitLiveAnswer) {
      onSubmitLiveAnswer(poll.id, option, isCorrect);
    }
  ...
  const handleCastWrittenAnswer = (poll: LiveQuestion) => {
    ...
    if (onSubmitLiveAnswer) {
      onSubmitLiveAnswer(poll.id, typedText, true);
    }
  ...
  const calculateOptionStats = (poll: LiveQuestion, opt: string) => {
    ...
    const userVote = studentVoteMap[poll.id] || responses.find((r) => r.studentId === currentStudent.id)?.answer;
    const isLocalNewVote = userVote === opt && !responses.some((r) => r.studentId === currentStudent.id);

    let count = responses.filter((r) => r.answer === opt).length;
    if (isLocalNewVote) {
      count += 1;
    }
    const total = responses.length + (userVote && !responses.some((r) => r.studentId === currentStudent.id) ? 1 : 0);
  ...
  return (
    <div className="space-y-6 pb-12">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="space-y-2 max-w-xl">
            <div className="flex items-center gap-2">
              <span className="px-2.5 py-0.5 rounded-full bg-sky-500/30 border border-sky-400/40 text-sky-200 text-xs font-bold uppercase tracking-wider">
                {selectedBatch.type} • {selectedBatch.durationLabel}
              </span>
              <span className="px-2.5 py-0.5 rounded-full bg-amber-500/20 text-amber-300 text-xs font-bold flex items-center gap-1">
                ...
                {currentStudent.activeStreakDays} Day Streak!
              </span>
            </div>
            ...
            <p className="text-xs sm:text-sm text-slate-300">
              You are currently enrolled in <strong>{selectedBatch.name}</strong> at{" "}
              {currentStudent.college || selectedBatch.college}. Your overall accuracy is at{" "}
              <strong>{currentStudent.scores?.overallAccuracy ?? 0}%</strong>.
            </p>
          </div>
          ...
          <div className="flex flex-wrap sm:flex-nowrap gap-3">
            <div className="bg-white/10 backdrop-blur-md px-4 py-3 rounded-2xl border border-white/10 text-center min-w-[100px]">
              ...
              <span className="text-xl font-black text-white">{currentStudent.totalPoints ?? 0}</span>
            </div>
            ...
            <div className="bg-white/10 backdrop-blur-md px-4 py-3 rounded-2xl border border-white/10 text-center min-w-[100px]">
              ...
              <span className="text-xl font-black text-amber-300">
                {currentStudent.attendedSessions}/{currentStudent.totalSessions}
              </span>
            </div>
          </div>
        </div>
      ...
      {/* ── QUICK TIME CLOCK & DAILY LOG LAUNCHER ── */}
      <div className="bg-white rounded-3xl border border-slate-200 shadow-xs p-5 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <span
                className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${
                  latestPunch?.type === "clock_in"
                    ? "bg-emerald-50 text-emerald-700 border border-emerald-200 animate-pulse"
                    : "bg-slate-100 text-slate-600"
                }`}
              >
                {latestPunch?.type === "clock_in" ? "Clocked In (Active)" : "Not Clocked In"}
              </span>
            </div>
            <h3 className="text-sm sm:text-base font-black text-slate-900 mt-0.5">
              Time Clock, Shift Telemetry & Daily Activity Log
            </h3>
            <p className="text-xs text-slate-400">
              Swipe in/out daily to track working hours, log deliverables, and avoid punch errors.
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2.5 w-full md:w-auto">
          <button
            ...
            className="flex-1 md:flex-none px-4 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold transition shadow-xs flex items-center justify-center gap-2 cursor-pointer"
          >
          ...
          <button
            ...
            className="flex-1 md:flex-none px-4 py-2.5 rounded-xl bg-slate-50 hover:bg-slate-100 text-slate-700 border border-slate-200 text-xs font-bold transition flex items-center justify-center gap-2 cursor-pointer"
          >
            <span>Daily Log</span>
          </button>
        </div>
      ...
      {/* ========================================================================= */}
      {/* LIVE COHORT SYNC CALL (REAL-TIME: ONLY SHOWN IF CALL IS SCHEDULED/LIVE)  */}
      {/* ========================================================================= */}
      {settings && (
        <LiveSessionBanner
          settings={settings}
      ...
      {/* ========================================================================= */}
      {/* CLIENT OPPORTUNITIES & APPROVED INTERVIEW ALERTS */}
      {/* ========================================================================= */}
      {internInterviews.length > 0 && (
        <div className="bg-gradient-to-r from-teal-600 via-emerald-600 to-teal-700 rounded-3xl p-6 text-white shadow-xl shadow-teal-700/10 flex flex-col md:flex-row items-start md:items-center justify-between gap-5 border border-teal-500/30">
          <div className="flex items-start gap-4">
            <div className="w-13 h-13 rounded-2xl bg-white/15 backdrop-blur-md text-white flex items-center justify-center flex-shrink-0 border border-white/20 shadow-xs">
              <Building2 className="w-7 h-7 text-amber-300" />
            </div>
            <div className="space-y-1">
              <div className="inline-flex items-center gap-2 px-2.5 py-0.5 rounded-full bg-white/20 text-white text-[10px] font-black uppercase tracking-wider">
                <Sparkles className="w-3 h-3 text-amber-300" />
                <span>Hiring Alert • {internInterviews.length} Opportunity</span>
              </div>
              <h3 className="text-lg font-black text-white leading-snug">
                {internInterviews[0].status === "scheduled"
                  ? `Confirmed Interview with ${clients.find(c => c.id === internInterviews[0].clientId)?.companyName || "Hiring Partner"}`
                  : `Hiring Partner requested an interview with you!`}
              </h3>
              <p className="text-xs text-teal-100 font-medium max-w-xl leading-relaxed">
                {internInterviews[0].status === "scheduled"
                  ? `Scheduled for ${new Date(internInterviews[0].scheduledDate!).toLocaleDateString("en-US", { weekday: "short", month: "short", day: "numeric" })} at ${new Date(internInterviews[0].scheduledDate!).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}. Click below to view details and join video call.`
                  : `Your technical profile was approved by the Program Administration. Click below to confirm your available time slot.`}
              </p>
            </div>
          </div>
          <button
            onClick={() => onNavigateTab("interviews")}
            className="px-6 py-3.5 bg-white text-teal-700 hover:bg-teal-50 font-black rounded-2xl text-xs shadow-lg transition flex items-center gap-2 flex-shrink-0 cursor-pointer"
          >
            <span>{internInterviews[0].status === "scheduled" ? "Join / View Interview" : "Pick & Schedule Slot"}</span>
        ...
        {effectivePolls.length > 0 ? (
          <div className="space-y-6">
            {effectivePolls.map((poll, qIdx) => {
              const existingResponse = poll.responses?.find((r) => r.studentId === currentStudent.id);
              ...
              return (
                <div
                  key={poll.id}
                  className="space-y-4 bg-slate-50/70 p-5 sm:p-6 rounded-2xl border border-slate-200/80 transition-all hover:border-slate-300"
                >
                      ...
                      {(poll.options || []).map((opt, oIdx) => {
                        ...
                        return (
                          <button
                            ...
                            className={`relative overflow-hidden text-left p-4 rounded-2xl border transition-all flex flex-col justify-between min-h-[76px] cursor-pointer ${buttonStyle}`}
                          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white p-6 rounded-3xl border border-slate-200/80 shadow-sm space-y-4 flex flex-col justify-between hover:shadow-md transition">
          <div className="space-y-2">
            ...
            <h3 className="font-black text-slate-900 text-lg">Interactive Learn Hub</h3>
        ...
        <div className="bg-white p-6 rounded-3xl border border-slate-200/80 shadow-sm space-y-4 flex flex-col justify-between hover:shadow-md transition">
          <div className="space-y-2">
            ...
            <h3 className="font-black text-slate-900 text-lg">Step-by-Step Assignments & IDE</h3>
            <p className="text-xs text-slate-500 leading-relaxed">
              Solve {batchAssignments.length} assigned challenge(s) with our one-by-one question navigator, white-theme in-browser IDE, and automated test cases.
            </p>
          </div>
        ...
        <div className="bg-white p-6 rounded-3xl border border-slate-200/80 shadow-sm space-y-4 flex flex-col justify-between hover:shadow-md transition">
          <div className="space-y-2">
            ...
            <h3 className="font-black text-slate-900 text-lg">Live Interactive Q&A</h3>
            <p className="text-xs text-slate-500 leading-relaxed">
              Participate in rapid-fire classroom polls, submit answers in real-time, ask trainer questions, and view instant leaderboard results.
            </p>
          </div>
