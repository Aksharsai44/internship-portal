...
export const LeaderboardView: React.FC<LeaderboardViewProps> = ({
    ...
    try {
      ...
      if (Array.isArray(res.data) && res.data.length > 0) {
        ...
        const backendStudents: Student[] = res.data.map((s: any) => ({
          ...
          fastestResponseMs: s.fastestResponseMs || 2500,
  ...
  // Sync with prop updates if live list is empty
  useEffect(() => {
    if (students.length > 0 && liveStudentsList.length === 0) {
      setLiveStudentsList(students);
    }
  }, [students]);
      ...
      <div className="bg-white rounded-3xl border border-slate-200/80 shadow-sm p-6 space-y-4">
        <div className="flex items-center justify-between">
          ...
          <span className="text-xs text-slate-400 font-medium">
            Click on any student to inspect 360° telemetry dossier
          </span>
        </div>
          ...
          <table className="w-full text-left text-xs sm:text-sm">
            <thead className="bg-slate-50 text-slate-500 font-extrabold uppercase tracking-wider text-[10px] border-b border-slate-200">
              <tr>
                ...
                <th className="py-3.5 pr-4 text-right">Total Score</th>
              </tr>
            </thead>
                ...
                filteredStudents.map((stu, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* Reaction Speed */}
                      <td className="py-3.5 px-3 font-mono text-slate-600 text-xs font-bold">
                        ⚡ {((stu.fastestResponseMs || 2000) / 1000).toFixed(2)}s
                      </td>
