        ...
        {batchAssignments.length > 0 ? (
          batchAssignments.map((asg) => {
            ...
            return (
              <div
                key={asg.id}
                className={`group flex items-center rounded-2xl border transition whitespace-nowrap overflow-hidden ${
                  isSelected
              ...
              >
                <button
                  ...
                  className="px-4 py-2.5 text-xs font-bold flex items-center gap-2 cursor-pointer"
                >
                  ...
                  >
                    {asg.durationMinutes}m • {asg.questions.length} Qs
                  </span>
                </button>

                {userRole === "admin" && (
                  <div
                    className={`flex items-center pr-2 gap-1.5 border-l pl-2 ${
                      isSelected ? "border-indigo-500/60" : "border-slate-100"
                    }`}
                  >
                    {/* Instant Lock / Unlock Button on Tab */}
                    <button
                      type="button"
                      onClick={(e) => {
                        e.stopPropagation();
                        handleToggleAssignmentLock(asg);
                      }}
                      className={`px-2 py-1 rounded-lg text-[10px] font-black border transition flex items-center gap-1 cursor-pointer shadow-2xs ${
                        asg.isLocked
                          ? "bg-rose-50 text-rose-700 border-rose-300 hover:bg-rose-100"
                          : isSelected
                          ? "bg-indigo-700 text-emerald-300 border-indigo-400 hover:bg-indigo-800"
                    ...
                    >
                      {asg.isLocked ? (
                        ...
                        <Unlock className={`w-3 h-3 ${isSelected ? "text-emerald-300" : "text-emerald-600"}`} />
                      )}
                    ...
                    <button
                      ...
                      onClick={(e) => {
                        e.stopPropagation();
                        handleDeleteAssignmentAction(asg);
                      }}
                      className={`p-1.5 rounded-lg text-xs font-bold transition cursor-pointer ${
                        isSelected
                          ? "hover:bg-rose-600 text-indigo-100 hover:text-white"
      ...
      <div id="assignment-fullscreen-container" className="bg-slate-50 overflow-y-auto">
        {selectedAssignment ? (
          userRole === "student" && !hasStarted ? (
            <div className="bg-white rounded-3xl border border-slate-200/80 shadow-sm p-12 space-y-6 text-center max-w-2xl mx-auto mt-8">
              <div className="w-20 h-20 bg-indigo-50 text-indigo-600 rounded-full flex items-center justify-center mx-auto mb-4">
                <FileText className="w-10 h-10" />
              </div>
              <h3 className="text-3xl font-black text-slate-900">{selectedAssignment.title}</h3>
              ...
              <div className="flex items-center justify-center gap-6 py-4">
                <div className="flex flex-col items-center">
                  ...
                  <span className="text-lg font-black text-slate-800">{selectedAssignment.durationMinutes} mins</span>
                </div>
                <div className="w-px h-10 bg-slate-200"></div>
                <div className="flex flex-col items-center">
                  <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">Questions</span>
                  <span className="text-lg font-black text-slate-800">{selectedAssignment.questions.length}</span>
                </div>
                <div className="w-px h-10 bg-slate-200"></div>
                <div className="flex flex-col items-center">
                  <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">Total Points</span>
                  <span className="text-lg font-black text-slate-800">{selectedAssignment.totalPoints}</span>
                </div>
              </div>
              <div className="bg-amber-50 text-amber-800 text-xs font-bold p-4 rounded-xl border border-amber-200 text-left flex items-start gap-3">
                ...
                <p>
                  <strong>Important Instructions:</strong> Once you click Start Assessment, the browser will enter full-screen mode.
                  Do not attempt to exit full-screen or switch tabs during the exam. If you exit full-screen, the assessment will be interrupted.
                </p>
              </div>
              <button
                onClick={handleStartAssessment}
                className="mt-6 w-full py-4 bg-indigo-600 hover:bg-indigo-700 text-white rounded-2xl text-lg font-black shadow-xl transition transform hover:-translate-y-1 flex items-center justify-center gap-2"
              >
            ...
            <div className="bg-white rounded-3xl border border-slate-200/80 shadow-sm p-6 sm:p-10 space-y-6">
              ...
              {selectedAssignment && userRole === "admin" && (
                <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200/90 shadow-sm space-y-6 mt-8">
                    ...
                    <div className="space-y-1">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        <h3 className="text-base sm:text-lg font-black text-slate-900">
                          Student Performance & Submission Analytics Hub
                        </h3>
                      ...
                      <p className="text-xs text-slate-500">
                        Real-time leaderboard, grading records, test case pass rates, and code inspection for{" "}
                        <strong className="text-slate-700 font-semibold">{selectedAssignment.title}</strong>
                      </p>
                    </div>
                    ...
                    <div className="flex items-center gap-2.5 flex-wrap">
                      ...
                      <div className="relative">
                        ...
                        <input
                          ...
                          className="pl-8 pr-3 py-1.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-medium focus:ring-2 focus:ring-indigo-500 w-48 sm:w-56"
                        />
                      </div>
                  ...
                  <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                    {/* Card 1: Submissions */}
                    <div className="p-4 bg-gradient-to-br from-indigo-50/80 to-slate-50 rounded-2xl border border-indigo-100/80 space-y-1">
                      ...
                      <div className="text-xl sm:text-2xl font-black text-slate-900">
                        {assignmentSubmissions.length} <span className="text-xs font-bold text-slate-400">Students</span>
                      </div>
                      <div className="text-[11px] font-bold text-indigo-600 flex items-center gap-1">
                        <CheckCheck className="w-3 h-3" />
                        <span>100% Graded in real-time</span>
                      </div>
                    </div>

                    {/* Card 2: Average Score */}
                    <div className="p-4 bg-gradient-to-br from-emerald-50/80 to-slate-50 rounded-2xl border border-emerald-100/80 space-y-1">
                      ...
                      <div className="text-xl sm:text-2xl font-black text-slate-900">
                        {assignmentSubmissions.length > 0
                          ? (
                              assignmentSubmissions.reduce((acc, s) => acc + (s.score || 0), 0) /
                              assignmentSubmissions.length
                            ).toFixed(1)
                          : "0"}{" "}
                        <span className="text-xs font-bold text-slate-400">/ {selectedAssignment.totalPoints || 30} Pts</span>
                      </div>
                    ...
                    {/* Card 3: Coding IDE Pass Rate */}
                    <div className="p-4 bg-gradient-to-br from-sky-50/80 to-slate-50 rounded-2xl border border-sky-100/80 space-y-1">
                        ...
                        {assignmentSubmissions.length > 0
                          ? Math.round(
                              (assignmentSubmissions.filter((s) => {
                                ...
                                return Object.values(s.codeSubmissions).some(
                                  (cs: any) => cs?.testResults?.allPassed
                                );
                              }).length /
                                assignmentSubmissions.length) *
                                100
                      ...
                      <div className="text-[11px] font-bold text-sky-600 flex items-center gap-1">
                        ...
                        <span>Compiler evaluated</span>
                      </div>
                    </div>

                    {/* Card 4: Top Performer */}
                    <div className="p-4 bg-gradient-to-br from-amber-50/80 to-slate-50 rounded-2xl border border-amber-100/80 space-y-1">
                      ...
                      <div className="text-[11px] font-bold text-amber-600 flex items-center gap-1">
                        ...
                        <span>
                          {assignmentSubmissions[0]?.score || selectedAssignment.totalPoints || 30} /{" "}
                          {selectedAssignment.totalPoints || 30} Pts (100%)
                        </span>
                      </div>
                    </div>
                  </div>

                  {/* Question-by-Question Accuracy Heatmap */}
                  <div className="p-4 sm:p-5 bg-slate-50 rounded-2xl border border-slate-200/80 space-y-3">
                    <div className="flex items-center justify-between">
                      <h4 className="text-xs font-extrabold uppercase tracking-wider text-slate-700 flex items-center gap-1.5">
                        <BarChart3 className="w-3.5 h-3.5 text-indigo-600" />
                        <span>Question Difficulty & Class Accuracy Breakdown</span>
                      </h4>
                      <span className="text-[10px] text-slate-400 font-bold">
                        {selectedAssignment.questions.length} Questions in Assessment
                      </span>
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                      {selectedAssignment.questions.map((q, qIdx) => {
                        ...
                        return (
                          <div
                            key={q.id}
                            className="p-3 bg-white rounded-xl border border-slate-200/70 shadow-2xs space-y-2"
                          >
                            <div className="flex items-center justify-between gap-2">
                              <div className="flex items-center gap-1.5 min-w-0">
                                <span className="w-5 h-5 rounded-md bg-indigo-100 text-indigo-700 text-[10px] font-black flex items-center justify-center flex-shrink-0">
                                  Q{qIdx + 1}
                                </span>
                                <span className="text-xs font-bold text-slate-800 truncate">
                                  {q.title || q.question.slice(0, 30)}
                                </span>
                              </div>
                            ...
                            <div className="space-y-1">
                              <div className="flex items-center justify-between text-[11px] font-bold">
                                ...
                                <span className="text-emerald-600 font-mono">{passPct}% Passed</span>
                              </div>
                      ...
                      <table className="w-full text-left text-xs">
                        <thead className="bg-slate-50/90 text-slate-500 uppercase font-extrabold text-[10px] tracking-wider border-b border-slate-200">
                          <tr>
                            ...
                            <th className="px-4 py-3">Student Name & College</th>
                            <th className="px-4 py-3">Status</th>
                            <th className="px-4 py-3">Score & Accuracy</th>
                            <th className="px-4 py-3">Coding Solution</th>
                                  ...
                                  <td className="px-4 py-3.5">
                                    {sub.codeSubmissions && Object.keys(sub.codeSubmissions).length > 0 ? (
                                      <div className="flex items-center gap-1.5">
                                        <span className="text-[10px] font-mono font-bold px-2 py-0.5 rounded-md bg-slate-900 text-emerald-400 border border-slate-800 flex items-center gap-1">
                                          <Terminal className="w-3 h-3 text-sky-400" />
                                          <span>All Tests Passed</span>
                                        </span>
                                      </div>
                                    ) : (
                                      <span className="text-[11px] text-slate-400 font-medium">MCQ Assessment</span>
                                    )}
                                  </td>
